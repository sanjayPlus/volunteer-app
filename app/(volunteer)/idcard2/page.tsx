...
function IdCreation() {
  ...
  const callImageApi = async () => {
    axios
      .get(`${VOLUNTEER_URL}/volunteer/download-logoV2?constituency=${loka}&assembly=${constituency}&booth=${booth}&mandalam=${assembly}`, {
        headers: {
          "x-access-token": localStorage.getItem("volunteer-token"),
        },
    ...
    <MobileContainer>
      <div className="w-full bg-[##F1F4FF] flex flex-col items-center relative bg-slate-100">
        ...
        <div className="w-[80%] gap-4 flex flex-col items-center justify-center ">
          {image ? <img src={`data:image/png;base64,${image}`} alt="Logo" />:<img src='/dmc/id-logo.png' alt="Logo" />}
